...
P, Q, R, S waves, are identified through inflection points in the ECG signal. The differences in amplitude between these points are critical in analyzing the signal morphology.

    In cases where the QRS complex exhibits complex morphology, the maximum signal value within a 100 ms window around the annotated R peak (𝑄𝑅𝑆𝑚𝑎𝑥) serves as a reference for accurately identifying the R peak. This adaptation is crucial for ensuring the reliability of feature extraction, especially in abnormal heartbeats.
